class SnipcartConfig < ApplicationRecord
  ...
  # Snipcart's loader, marked so Turbo won't run it again on every navigation.
  #
  # Turbo Drive re-executes <script> elements in the body it swaps in, and
  # Snipcart's loader can't tell it has already run: its check for an existing
  # copy is
  #
  #   document.querySelector('src[src^="..."][src$="snipcart.js"]')
  #
  # which selects a <src> tag — an element that doesn't exist — so it is always
  # null and the loader always appends another snipcart.js. Its only guard is a
  # closure flag private to one execution, and Turbo gives each execution a
  # fresh one. Four copies of Snipcart were loading on a browse through the
  # store, each with its own click handler, so one "Add to Cart" registered
  # four times.
  #
  # Snipcart persists on `window` once loaded, and Turbo keeps the JS context
  # across visits, so running the loader exactly once per full page load is
  # both sufficient and correct.
  TURBO_EVAL_OFF = ' data-turbo-eval="false"'

  def current_snippet_html
    return nil if current_snippet.blank?

    current_snippet
      .gsub(/<script(?![^>]*data-turbo-eval)/i) { |tag| "#{tag}#{TURBO_EVAL_OFF}" }
      .html_safe
  end
